...
const CONST = {
  ...
  fogFar: 3.20,      /* fog full, same units. Recession is carried by a gradient
                        that never reaches the field inside the frame. 1.90
                        saturated well within the scene, so everything past
                        roughly twice the camera distance lifted to pure white
                        and stopped being drawn at all: depth was implemented as
                        deletion, and it read as flatness rather than as space.
                        Measured on robotics at hero, going to 3.20 raised local
                        tonal modulation from 0.074 to 0.110 against 0.116 for
                        the pre-depth-pass render. Keep the far plane outside the
                        scene depth; do not tune it back down to make a ridge
                        recede, move the ridge. */
  dot: '#131312',    /* the interface ink exactly; a cooler dot read as a
                        different black next to type on the same page */
  field: '#FCFCFB',
  version: '1.3',    /* bump when a constant changes; published work keeps its own */
};
  ...
  if (o.ridge){
    ...
    if (o.ridgeDist == null){
      throw new Error(
        ...
        '(target fog fraction roughly 0.25 to 0.40) rather than deriving it from d.'
      );
    }
...
function buildScene(id){
  ...
  if (id === 'datacenter'){
    /* Racks stand in back-to-back pairs: a tight gap within a pair (the hot
       aisle) and a wide gap between pairs (the cold aisle). A data hall reads
       through that repeating gap, not through the racks themselves — compare
       port's skipped haulage-lane column, the one part of this file that
       already passed the recognition test on a rack-like subject. */
    const cabs = 9, depth = 2.0, rackH = 5.0, within = 1.2, between = 6.5;
    const gaps = [within, between, within, between, within, between, within];
    const x0 = [0];
    for (const gap of gaps) x0.push(x0[x0.length-1] + gap + depth);
    const xOff = x0[x0.length-1] / 2;
    const zOff = (cabs - 1) * 2.4 / 2;
    for (const xRaw of x0){
      const x = xRaw - xOff;
      for (let i=0;i<cabs;i++){
        const z = i*2.4 - zOff;
        /* Rack bodies sit on the two adjacent mass tones at the dark end and
           the 0.28-unit cap rail takes TONE[4], the deepest step. The rail is
           the thinnest element in the scene and the one that says "row of
           cabinets" rather than "block": it is what gives each corridor a
           per-cabinet cadence along its top edge instead of one unbroken
           silhouette. Reserving the deepest step for the thinnest defining
           element is the same call port makes for a gantry beam and grid
           makes for a conductor. Before this the racks drew from TONE[1] and
           TONE[2] and the rail from TONE[0], which left the subject spanning
           58 levels against every other subject's 128. */
        box(depth,rackH,1.9, x, rackH/2+0.5, z, rnd()>.7?TONE[3]:TONE[2]);
        box(depth,.28,1.9, x, rackH+0.64, z, TONE[4]);
      }
    }
    /* A wall at the far end of the corridors, in a tone the racks never
       reach, so the aisles resolve into an interior instead of open space —
       the envelope that says hall rather than a field of freestanding masses.
       It sits lighter than the racks now rather than level with them, which is
       the correct direction: a lit surface at the end of a corridor read
       against dark mass in front of it, the same relationship the TONE[1]
       floor has with the aisles. */
    box(x0[x0.length-1] - xOff + 6, rackH + 2.4, 0.6, 0, (rackH + 2.4)/2 + 0.5, -zOff - 1.5, TONE[1]);
    ground(g, TONE, rnd, { w: 120, d: 110, amp: 0.8, flat: 30, ridge: 4.0, ridgeDist: 17, scatter: 16 });
  }
  ...
  if (id === 'urban'){
    for (let i=0;i<17;i++) for (let j=0;j<13;j++){
      ...
      /* Written out as materials rather than as an index computed into TONE,
         so the per-subject tone scan in tests/dot-engine.test.ts can see which
         steps this subject draws from. Same materials in the same order, and
         rnd() is still reached in exactly one branch, so the render is
         byte-identical to the index form: verified against the shipped
         urban-card-dot.png before this landed. */
      const tone = cz < -8 ? TONE[0] : cz < 2 ? TONE[1] : (rnd() > .6 ? TONE[3] : TONE[2]);
      box(2.7,h,2.7, cx, h/2, cz, tone);
    }
